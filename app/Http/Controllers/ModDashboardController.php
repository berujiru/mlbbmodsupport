...
class ModDashboardController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        $infra_mod = Markdowns::select('Form_Attribute',
            DB::raw('SUM(`Infractions`) AS infractions'))
            ->where('Form_Attribute','<>',"'Form Attribute'")
            ->where('MOD_ID','=',$dbsc->modid)
            ->groupby('Form_Attribute')
            ->orderByRaw(DB::raw("Form_Attribute")." ASC")
            ->get();
        ...
        $infra_form_attrib = Markdowns::select('Form_Attribute',
            ...
            DB::raw("(
                ...
                END) AS attrib_name"),
            DB::raw('SUM(`Infractions`) AS infractions'),
            DB::raw('GROUP_CONCAT(`Date`) AS date_committed'))
            ->where('Team','<>',"'Team'")
            ->where('MOD_ID','=',$dbsc->modid)
        ...
        // echo "</pre>";
        // exit;
        return view('index-mod',compact('dbsc','overall_score','accuracy_score','timeliness_score','communication_score','infra_mod','overall_team_summary','greeting','list_attributes','infra_form_attrib','summary_infra_mo_yr','summary_infra_attrib','greeting_name'));
        //return view('index');
    }
