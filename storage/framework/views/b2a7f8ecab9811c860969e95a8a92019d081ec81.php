      ...
      <tbody>
        <?php $__empty_1 = true; $__currentLoopData = $data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $score): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
          <tr>
              ...
              <td style="width:10%;">
                <?php if($from != '' && $to != ''): ?>
                  <a class="btn btn-sm btn-info" href="<?php echo e(route('score-summary.show',['id'=>$score->MOD_ID,'type_summary'=>$type_summary,'from'=>$from,'to'=>$to,'mo_yr'=>(!empty($score->month_yr) ? $score->month_yr : '')])); ?>" title="View Scores"><i class="bx bx-fw bx-show bx-xs"></i></a>
                <?php else: ?>
